...
public class TestClass {
	...
	public void testChisq() {
		...
		int size = 1;
		ChiSqTestResult chisq = Main.computeChisq(input, size);
		...
		int size2X3 = 2;
		ChiSqTestResult chisq2X3 = Main.computeChisq(input2X3, size2X3);
		...
		int size2 = 1;
		ChiSqTestResult chisq2 = Main.computeChisq(input2, size2);
		double result2 = chisq2.statistic();
		assertEquals(50.2304, result2, DOUBLE_MIN);
	...
	public void itemCounter() throws FileNotFoundException, UnsupportedEncodingException {
		...
		double[] result = Main.filterItem(basket, transactions);
